	...
	while (q--) {
		...
		for (int i = 0; i < 31; i++) {
			...
			if (l  == 0)
				...
				p = bits[r][i] - bits[l - 1][i]; // number of ones
			if (p < total)// zero zada then one
				x1 |= (1LL << i);
			if (p != (r - l + 1)) // agr pura one nhi tab ik  one
				x2 |= (1LL << i);
			if (p) // khi pe bhi one aa rha tho bachane k liye one
				x3 |= (1LL << i);
		}
