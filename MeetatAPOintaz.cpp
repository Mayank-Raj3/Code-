...
void solve() {
	...
	/*
		as we know that . . . . x . . y we want a point such that sum of x-xi is min that will be the
		median
		so by using that fact we can use that here for y and x independetly and calculate the diff
	...
	*/
	...
	int xofmeetpoint , yofmeetpoint;
	xofmeetpoint = x[n / 2] ;
	yofmeetpoint = y[n / 2];

	int ans = 0 ;
	for (auto it : x ) {
		ans += abs(xofmeetpoint - it);
	}
	for (auto it : y ) {
		ans += abs(yofmeetpoint - it);
	}
	cout << ans << nline ;
