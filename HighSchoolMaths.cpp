...
void solve() {
    ...
    int a , b , n ; cin  >> a >> b >> n ;
    // int prev = mod_add(a , b, mod), ans = prev , res = 0 ;
    // for (int i = 2 ; i <= n  ; i++) {
    //     int t  = (mod_mul(a, prev, mod));
    //     prev = mod_add(t, b, mod) ;
    //     ans = mod_add(prev, ans, mod);
    // }
    // cout << prev << nline;

    int po = binpow(a, n, mod);

    if (n == 1) {
        cout << mod_add(a, b, mod) << nline; return ;
    }

    if (a == 1) {
        cout << mod_add(1, mod_mul(b, (n), mod), mod) << nline;
        return ;
    }
    int num = mod_mul(b, mod_sub( 1, po , mod), mod);
    int base = mod_sub(1 ,  a, mod);
    int frac = mod_div(num, base, mod);
    int ans  = mod_add(frac, po, mod);
    cout << ans << nline;
}
int32_t main() {
#ifndef ONLINE_JUDGE
