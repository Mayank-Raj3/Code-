...
struct concatKarr {
	...
	void build(vector<int> arr) {
		...
		tot = 0;
		for (int i = 0 ; i < n ; i++) {
			tot = mod_add(tot, arr[i], mod);
		}

		prefBest = arr[0] , sum = arr[0];
		for (int i = 1 ; i < n ; i++) {
			sum = mod_add(sum, arr[i], mod);
...
concatKarr merge(concatKarr a , concatKarr b) {
	...
	ans.best = max({a.best % mod, b.best % mod, (mod_add(a.suffBest , b.prefBest, mod))});
	ans.prefBest = max(a.prefBest % mod , (mod_add(a.tot , b.prefBest, mod)));
	ans.suffBest = max(b.suffBest % mod , (mod_add(b.tot , a.suffBest, mod)));
	ans.tot = mod_add(a.tot , b.tot, mod);
