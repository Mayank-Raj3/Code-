...
/*
You are given a list of N intervals. The i-th interval
is [li, ri), which denotes a range of numbers greater than
or equal to li and strictly less than ri.
Find the minimum integer x such that you can select x
intervals from the given N intervals so that the union of
the selected intervals is [0, L).


Input Format

The first line contains two integers N (1 ≤ N ≤ 2×105) and
L (1 ≤ L ≤ 1012), where N is the number of intervals and
L is the length of the range to be covered, respectively.
...
void solve() {
	...
	sort(all(events));
	int cnt = 0 , mx = 0, end = 0, i = 0;
	while (i < n) {
		while (i < n and  events[i].ff <= end) {
			mx  = max(mx , events[i].ss);
			i++;
		}
		cnt++;
		end = mx ;
