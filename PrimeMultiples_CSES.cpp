...
void solve() {
	...
	ll cnt = 0;
	/*
	Genraliszed way when all are not prime then also it works
	Not working tle
		...
		for (int i = 0; i < k; i++)
			if (mask & (1 << i)) {
				...
				if (lcm == 0)
					...
					lcm = (1ll * lcm * prime[i]) / gcd(lcm, (ll)prime[i]);
			}
	...
	*/
	...
	// since only primes is here of in that case lcm is just the multiplication
	// A U B U C = A + B+ C -(A∩B)-(A∩C)-(B∩C) + (A∩B∩C)
	//  frm this we notice that even is getting - and odd +


	for (int mask = 1; mask < (1 << k); mask++) {
		ll  setBit = __builtin_popcount(mask), prod = 1ll ;
		for (int i = 0; i < k; i++) {
			if (mask & (1 << i)) {
				// optimisztion

				// if (prod*prime[i] > n ) bhut this may overflow so we divide there
				if (prod > n / prime[i]) {
					// jab prime[i] * product(p0,p1,....prime[i-1])  >n then ofc => n/prod = 0 so we break early without overflowing the nums
					prod = n + 1 ;
					break ;
				}
				prod *= prime[i];
