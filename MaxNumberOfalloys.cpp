...
int maxNumberOfAlloys(int n, int k, int budget, vector<vector<int>>& composition, vector<int>& stock, vector<int>& cost) {
	...
	auto check = [&](int mid) {
		...
		for (int i = 0 ; i < k ; i++) {
			...
			bool made = true ;
			for (int j = 0  ; j < n ; j++) {
				int req = (1ll * mid * composition[i][j]) - stock[j] ;
				money += (req * cost[j]);
			}
			if (money <= budget) {
				return true ;
...
void solve() {
	int n , k , budget ;
	cin >> n >> k >> budget;
	vector<vector<int>> composition(k, vector<int>(n));
	vector<int> stock(n), cost(n);
	for (int i = 0 ; i < k ; i ++) {
		for (int j = 0 ; j < n ; j++) {
			cin >> composition[i][j];
		}
	...
	cout << maxNumberOfAlloys(n, k, budget, composition, stock, cost);
