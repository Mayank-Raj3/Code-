...
Perform the operations optimally such that
∑array[i]*array[i] for all 1<=i<=n is maximum.
 Print the largest sum of squares you can get
 after performing the operations greater than equal
  to zero times.
...
void solve() {
	...
	/*
		by or :- we can fill the 0 taking from another number
		and :- will make the used one 0

		so we can shift one from one number to another

		so we can maximize the ans by greedily making all bit one
