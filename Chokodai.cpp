...
int rec(int ind  , int wordInd) {
	if (wordInd >= word.size()) return 1 ;
	if (ind > s.size()) return 0 ;

	if (dp[ind][wordInd] != -1) return dp[ind][wordInd];

	int ans = rec(ind + 1 , wordInd); //not take
	if (s[ind] == word[wordInd]) {
		ans += (rec(ind + 1 , wordInd + 1));
	}
	return dp[ind][wordInd] = ans ;
}
