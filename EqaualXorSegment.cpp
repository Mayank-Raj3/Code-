...
vector<int> arr, prefXor;
map<int, vector<int>> xorMpp;

int getInd(int reqXor, int l) {
	if (!xorMpp.count(reqXor))
		return INF;

	const auto &v = xorMpp[reqXor];
...
void solve() {
	...
	xorMpp[0].push_back(0);

	for (int i = 1; i <= n; i++) {
		cin >> arr[i];
		prefXor[i] = prefXor[i - 1] ^ arr[i];
		xorMpp[prefXor[i]].push_back(i);
	}

	while (q--) {
		int l, r; cin >> l >> r;

		int A = (prefXor[l - 1] ^ prefXor[r]);
		...
		if (A == 0) {
			...
			/*
			 xor not eqal to 0 k = 3

			when xor > 0
			[1 -> l-1][l^r=A]
			_____________________________________________________

			[l ---  i][i+1 ---- j][this must be A]

			A= pref[l-1]^pref[l]

			1. find smallest i >=l and xor bettwn i to l = A

			pref[l-1]^pref[i] = A
			=> pref[i] = pref[l-1]^A = k  // main


			2. find smallest j >=i and j ^ i  = A

			pref[i]^pref[j] = A
			pref[j] = pref[i]^A  // main

			*/


			int i = getInd((A ^ prefXor[l - 1]), l - 1);
			if (i >= r) {
				NO
				continue;
			}

			int j = getInd((A ^ prefXor[i]), i);
			if (j >= r) {
				NO
				continue;
			}
