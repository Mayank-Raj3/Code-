...
void solve() {
	...
	vector<int> depta (n + 1);
	vector<int> parent (n + 1);
	vector<int> subtreesz (n + 1);

	function<void(int, int, int)> dfs = [&](int node, int depth, int par) {
		depta[node] = depth;
		parent[node] = par;
		subtreesz[node] = 1;
	...
	// A centroid of a tree is defined as a node such that
	// when the tree is rooted at it, no other nodes have a
	// subtree of size greater than n/2
	dfs(1, 0, 0);
	db(subtreesz);
	function<int(int, int)> get_centroid = [&](int node, int par) {
		for (int i : adj[node]) {
			...
			if (subtreesz[i] * 2 > n) { return get_centroid(i, node); }
		}
		return node;
	};
	cout << get_centroid(1, 0) << nline ;
}
int32_t main() {
#ifndef ONLINE_JUDGE
