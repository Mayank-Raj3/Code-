...
void solve() {
	...
	int room = 0 ;
	for (int i = 0 ; i < n ; i++) {
		int currAr = arr[i][0] , Currdep = arr[i][1] , ind = arr[i][2];

		if ((ms.size() == 0 ) or (ms.begin()->ff >= currAr)  ) {
			// no vacant
			room++;
			ms.insert({ Currdep, room });
			ans[ind] = room;
		} else {
			// vacant room when arrv time is > dep time
			int Vacroom = ms.begin()->ss;
			ms.erase(ms.begin());
			ms.insert({ Currdep, Vacroom });
			ans[ind] = Vacroom;
		}
	}
	cout << room << nline;
