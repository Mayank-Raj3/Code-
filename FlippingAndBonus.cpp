...
int rec(int ind, int cnt, vector<int> &arr, map<int, int> &mpp, vector<vector<int>> &dp) {
	if (ind >= arr.size()) {
		return 0;
	}
	...
	int take = rec(ind + 1, cnt + 1, arr, mpp, dp) + arr[ind] + mpp[cnt + 1];
	int nottake = rec(ind + 1, 0, arr, mpp, dp) + 0;
	return dp[ind][cnt] = max(nottake, take);
}
void solve() {
	int n , m ; cin >> n >> m ;
	vector<int> arr(n);
