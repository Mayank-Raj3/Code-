...
void solve() {
	...
	auto func = [&](int k) {
		// O(n)
		vector<int> tempo = arr; // temp array

		int ans = 0 ;

		for (int bit = 60; bit >= 0 ; bit--) {
			int temp = 0 ; // cost we req

			for (int i = 0 ; i < n ; i++) {
				if (tempo[i] & (1ll << bit)) {
					// if that bit pos is already one
					continue;
				} else {
					// use previsous number + extra ops
					temp += ((1ll << bit) - tempo[i]); // cost when bit is not one
				}
				if (temp > k) break ; // if at any point of time cost becomes great than the ops we have we break
			}

			if (temp <= k) {
				// if cost to make that all element ' s ith bits set is less than no of ops we take that to our ans
				k -= temp; // we reduce the ops
				ans = ans | (1ll << bit);
				for (int x = 0 ; x < n ; x++) {
					//  set to one

					// if already set the shift and not use previos
					// else we use previous and make it zero
				...
				// if we moved forwad without applying operation
				// that bit will beocome 0 if thats one
				for (int x = 0 ; x < n  ; x++) {
					// we discard that bit and set to zero
					// since after this we wont be able to use this
					if (tempo[x] & (1ll << bit)) tempo[x] ^= (1ll << bit);
				}
			}
