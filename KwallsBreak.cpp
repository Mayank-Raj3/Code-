...
void solve() {
	...
	while (!q.empty()) {
		int x, y, bomb;
		tie(x, y, bomb) = q.front();
		q.pop();

		for (int i = 0; i < 4; i++) {
			int nx = x + dx[i];
			int ny = y + dy[i];
			int nbomb = bomb + (arr[nx][ny] == '#');

			if (isValid(nx, ny) && nbomb <= k && dis[nx][ny][nbomb] == 1e9) {
				dis[nx][ny][nbomb] = dis[x][y][bomb] + 1;
				q.push({nx, ny, nbomb});
			}
		}
	}
	...
	for (int bomb = 0; bomb <= k; bomb++) {
		ans = min(ans, dis[end.ff][end.ss][bomb]);
	}
