...
void merge(int l , int mid , int r ) {
	...
	// upper-arr[i] >= x ==>  # of ele>=x
	// low-arr[i] <=x ==>  # of ele <=x
	...
	for (int i = mid  ; i >= l ; i--) {
		...
		int left = lower_bound(pref.begin(), pref.end(), sum - up) - pref.begin();
		int right = upper_bound(pref.begin(), pref.end(), lo - sum) - pref.begin();
		ans += (right - left + 1);
...
void solve() {
	...
	int lower , upper;
	cin >> lower >> upper;
	cout << countRangeSum(arr, lower, upper);
