...
int rec(int  i, int j , int x ) {
	...
	int ans = 0 ;
	if (arr[i][j] and x < k) {
		ans = mod_add(ans, mod_add(rec(i - 1, j, x + 1) , rec(i, j - 1, x + 1), mod), mod);
	} else {
		ans = mod_add(ans, mod_add(rec(i - 1, j, x ) , rec(i, j - 1, x ), mod), mod);
	}
