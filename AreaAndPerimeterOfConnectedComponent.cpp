...
void solve() {
	...
	auto isvalid = [&](int x , int y ) {
		if (x >= 0 and y >= 0 and x < n and y < n and vis[x][y] != 1 and arr[x][y] == '#')
			return true ;
		return false ;
	};
	...
	auto isbon = [&](int x , int y ) {
		if (x >= 0 and y >= 0 and x < n and y < n and arr[x][y] == '#' )
			return false ;
		else
			return true ;
	};
	...
	for (auto it : compo ) {
		...
		for (auto it2 : it) {
			for (int k = 0 ; k < 4 ; k++) {
				int x = it2.ff + dx[k] , y = it2.ss + dy[k];
				if (isbon(x, y)) {
					tp++;
				}
		...
		if (ta == area and peri > tp) {
			area = ta;
			peri = tp;
		}
	}

	cout << area << " " << peri << nline ;
