...
/*
`State :- dp[i][j] represents lcs till a[0,..i]. and b[0,..j] `
`Transition : - dp[i][j]  if a[i]==b[i] :   1+ dp[i-1][j-1] since its subsequnce so we can include the previos lcs  `
`if not equal include max of previous states :- max{dp[i][j-1] , dp[i-1][j]} `
