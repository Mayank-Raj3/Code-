...
void solve() {
	...
	q3.
		...
		Method 1 : - using partialsum on segtree
	*/
	...
	while (q--) {
		...
		if (t == 1) {
			...
			int pos ; cin >> pos ;
			pos--;
			// note to query from 0 to pos so that values in that range can be added like partial sum
			cout << query(1, 0, n - 1, 0, pos ) + arr[pos] << nline;
		}
