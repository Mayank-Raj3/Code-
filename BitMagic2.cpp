...
2.Output the position of the most significant bit ( MSB )
or the leftmost set bit of n ( 0- indexed ). If n=0, output -1.

3.Output the position of the rightmost set bit of n ( 0- indexed ).
 If n=0, output -1.

4.Output 1 if n  is a power of 2 i.e n==2X , x > 0, else output 0.

5.Output the biggest power of 2, that is the divisor of n.
i.e biggest 2K such that n%2K == 0, k ≥ 0. If n=0, output -1.

6.Output the smallest power of 2, which is not smaller than n,
 i.e smallest 2K such that 2K ≥ n, k>0.
*/
int n ;
void print_bin() {
	for (int i = 63 ; i >= 0 ; i--) {
...
void smdivs() {
	...
	if (n <= 1) cout << 2 << nline;
		...
		cout << (1LL << (64 - __builtin_clzll(n - 1))) << nline;
